...
#[derive(Debug)]
pub(super) struct BlockBitmap<'a>(pub(super) &'a [u8]);
...
impl<'a> BlockBitmap<'a> {
    ...
    /// bit) of byte 1.
    #[allow(dead_code)]
    pub(super) fn is_used(&self, block: BlockAddress) -> Option<bool> {
        let index = block.0 / 8;
...
#[derive(Debug)]
pub(super) struct InodeBitmap<'a>(pub(super) &'a mut [u8]);
