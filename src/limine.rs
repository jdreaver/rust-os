use limine::{LimineBootInfoRequest, LimineFramebufferRequest, LimineMemmapEntry, LimineMemmapRequest, NonNullPtr};
...
pub fn print_limine_boot_info() {
    let boot_info = BOOT_INFO_REQUEST.get_response().get().expect("failed to get limine boot info");
    ...
    let boot_info_version_ptr = boot_info.version.as_ptr().expect("no limine boot info version");
...
pub fn print_limine_memory_map() {
    let memory_map = MEMORY_MAP_REQUEST.get_response().get().expect("failed to get limine memory map");
    ...
    for i in 0..memory_map.entry_count {
        unsafe {
            ...
            serial_println!("    base: {:#x}, len: {:#x}, type: {:?}", entry.base, entry.len, entry.typ);
        }
    }
}
